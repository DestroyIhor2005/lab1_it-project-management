...
test('має повертати валідні Binance-монети навіть якщо частина збігів не торгується', async () => {
  const originalFetch = global.fetch;
  resetApiCaches();
  ...
  global.fetch = async (url) => {
    ...
    if (requestUrl.includes('/coins/markets?vs_currency=usd&ids=pepe,wrapped-xpl')) {
      return {
        ok: true,
        json: async () => ([
          {
          ...
          },
          {
            ...
            name: 'Wrapped XPL',
            symbol: 'wxpl',
            image: 'https://example.com/wxpl.png',
            current_price: 0.123,
  ...
  try {
    ...
    assert.equal(results[1].symbol, 'WXPL');
    assert.equal(results[1].price, 0.123);
  } finally {
    global.fetch = originalFetch;
  }
});
...
test('має додавати результати з локального каталогу якщо CoinGecko search не повернув потрібний токен', async () => {
  ...
  global.fetch = async (url) => {
    ...
    if (requestUrl.includes('/coins/markets?vs_currency=usd&ids=gold,tether-gold')) {
      return {
        ok: true,
        json: async () => ([
          {
...
test('має брати ціну з Binance для пошуку, якщо CoinGecko price endpoints вже недоступні', async () => {
  ...
  global.fetch = async (url) => {
    ...
    if (requestUrl.includes('/ticker/24hr?symbol=SOLUSDT')) {
      return {
        ok: true,
        json: async () => ({
          symbol: 'SOLUSDT',
